...
class modRSContactHelper{
	...
	public static function getAjax(){
		...
		try{
			...
			// send admin email
			$sent_admin = $mail->sendMail($email, $sender, $recipient, $subject_predef, $msg, $mode = true, $cc, $bcc, $attachment = null, $replyTo, $replyToName = null);
		
			// send selfcopy email
			if ($selfcopy || $send_copy) { 
				$subject 	= JText::sprintf('MOD_RSCONTACT_SEND_COPY_SUBJECT', $config->get('sitename'));
				$not_sent 	= JText::sprintf('MOD_RSCONTACT_ADMIN_EMAIL_NOT_SENT', '<strong>'.$email.'</strong><br />');
				
				$sent_user = JFactory::getMailer()->sendMail($config->get('mailfrom'), $config->get('fromname'), $email, $subject, $not_sent.$msg, true);
				if ($sent_user !== true) {
			...
			return self::showResponse(1, $thank_you, $warning); 
		} catch (Exception $e) {
			return self::showResponse(0, $e->getMessage());
		}
	}
}