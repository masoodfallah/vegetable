...
<div id="mod-rscontact-container-<?php echo $uniqid;?>">
    <form id="mod-rscontact-contact-form-<?php echo $uniqid; ?>" name="mod-rscontact-contact-form" enctype="multipart/form-data" method="post" action="" <?php if($form_horizontal) echo 'class="form-horizontal"';?>>
		<fieldset>
		<?php echo JHtml::_( 'form.token' ); ?>
		<input type="hidden" id="mod-rscontact-module-id" value="<?php echo $module->id; ?>" />
		<input type="hidden" name="mod-rscontact-module-name" value="<?php echo $module->title; ?>" />
		...
		<?php if ($show_state) { ?>
			<?php if ($show_state_req) {
				...
				foreach (modRSContactHelper::$states as $state) {
						$data_state[] = JHtml::_('select.option', $state[0], $state[1]);
				} ?>	
				<div class="control-group">
			<?php if ($show_label) { ?>
		...
		<?php if ($show_subject) { ?>
			...
			<?php	case "textarea": ?>
						<textarea rows="5" name="mod_rscontact_subject" id="mod-rscontact-subject-<?php echo $uniqid; ?>" value="" class="<?php echo $class; ?>" <?php echo $placeholder; ?> ></textarea> 
				<?php  break; ?>
			...
			<?php	case "select":
						if ($subject_input) {
							...
							echo JHtml::_('select.genericlist', $data_subject, 'mod_rscontact_subject', 'class="'.$class.'"', 'value', 'text');
	...
	<?php require JModuleHelper::getLayoutPath('mod_rscontact', 'default_custom'); ?>
		...
		<?php } ?>
			<div class="controls">
				<textarea name="mod_rscontact_message" id="mod-rscontact-message-<?php echo $uniqid; ?>" value="" rows="5" class="<?php echo $class;?>" <?php echo $placeholder;?> ></textarea>
			</div>
		</div>	
		<?php } ?>
	...
	<?php }  ?>
			...
			<div class="controls">
				<input type="submit" id="mod-rscontact-submit-btn-<?php echo $uniqid;?>" name="mod_rscontact_submit-btn-<?php echo $uniqid;?>" value="<?php echo JText::_('MOD_RSCONTACT_SUBMIT_LABEL');?>" id="submit" class="btn btn-primary submit"  />
				<img id="loading-<?php echo $uniqid;?>" class="mod-rscontact-loader" alt="Loading, please wait.." src="media/mod_rscontact/images/ajax-loader.gif" />
			</div>
